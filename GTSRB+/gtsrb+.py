...
# --- Standard ResNet44 Model (Z2) ---
class ResNet44_Z2(nn.Module):
    def __init__(self, block=BasicBlockZ2, num_blocks_per_stage=[7, 7, 7], num_classes=43, input_channels=3):
        super().__init__()        
        self.in_channels = 32
    ...
    def forward(self, x):
        ...
        out = self.fc(out)           # Classify -> (B, num_classes)
...
class ResNet44_p4m_e2cnn(torch.nn.Module):
    r"""ResNet‑44 with p4m equivariance on GTSRB."""
    def __init__(self, n: int = 7, num_classes: int = 43): 
        super().__init__()

        # ---------------- gspace & field helpers --------------------------
        self.r2_act = gspaces.FlipRot2dOnR2(N=4)            # p4m
        channels    = [11, 23, 45]                         

        self.in_type = enn.FieldType(
            self.r2_act,
            3 * [self.r2_act.trivial_repr]                  # RGB input
        )
        ...
        self.avgpool    = torch.nn.AdaptiveAvgPool2d(1)               # spatial avg
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Wrap vanilla tensor into a GeometricTensor
        ...
        x = self.gpool(x)           # shape: (B, 45, H, W)
        x = self.avgpool(x.tensor)  # → (B, 45, 1, 1)
        x = torch.flatten(x, 1)     # → (B, 45)

        return self.fc(x)

"""# P4m-W RestNet44 model"""
...
class P4mW_ResNet44(torch.nn.Module):
    r"""ResNet‑44 with p4m equivariance on CIFAR‑10."""
    def __init__(self, n: int = 7, num_classes: int = 43):
        super().__init__()
...
def get_rotated_cifar10(batch_size=128, seed=42):
    mean = (0.3582, 0.3628, 0.3567)
    std = (0.2075, 0.2134, 0.2099)
    ...
    full_train = datasets.GTSRB(root='./data', split='train', download=True, transform=transform_train)
    test_set = datasets.GTSRB(root='./data', split='test', download=True, transform=transform_test)

    # Split into 40k train + 10k val
    generator = torch.Generator().manual_seed(seed)
    train_set, val_set = random_split(full_train, [21312, 5328], generator=generator)
...
def training_loop(model, device, train_loader, test_loader, val_loader, hyper_parameters):
    ...
    try:
        ...
        print(f"\nFinal Test Accuracy (Best {type(model).__name__} Model): {test_accuracy:.2f}%")
...
hyper_parameters = {
  ...
  "SEED": 42,
  "BEST_MODEL_PATH": "z2cnn_best_tuned.pth"
}
